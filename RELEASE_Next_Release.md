...
*   **Mod List Tooltip Description Fix**: Fixed a bug where hovering over a mod card in the mod list showed "No description provided." in the tooltip even when a description existed in the mod's metadata. Description, link, and nexus_id fields are now cached properly in memory when scanning mods.
